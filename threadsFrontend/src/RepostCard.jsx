  ...
  const handleRepost = async (e) => {
    ...
    try {
      ...
      if (result.reposted) {
        ...
        console.log('RepostCard: Repost successful, calling onRepostUpdate');
        if (onRepostUpdate) onRepostUpdate(Date.now()); // Trigger profile refresh
      } else {
        setRepostCount(prev => Math.max(0, prev - 1));
        console.log('RepostCard: Unrepost successful, calling onRepostUpdate');
  ...
  return (
    <>
      <div className="bg-neutral-800 rounded-xl p-4 flex gap-4 text-left w-full max-w-xl mx-auto mb-6 shadow-sm border border-neutral-700 relative group">
        {/* Repost header */}
        <div className="absolute top-2 left-4 text-xs text-neutral-400 flex items-center gap-1">
          <span>🔁</span>
          <span>{repost.reposter?.username} reposted</span>
          <span>· {new Date(repost.createdAt).toLocaleString()}</span>
        </div>
        
        {/* Avatar with follow/unfollow button in the corner */}
        <div className="relative w-10 h-10 mt-8">
          <img src={originalPost.User?.profilePicture || 'https://i.pravatar.cc/40'} alt="avatar" className="w-10 h-10 rounded-full object-cover" />
